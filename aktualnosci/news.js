...
function renderSinglePost(post) {
  ...
  document.querySelector(".news-footer").style.display = "none";
  document.querySelector(".banner-short").style.display = "none";
  document.getElementById("breadcrumbList").style.display = "none";
...
<section class="banner" style="margin: 0;">
    <div class="banner-image" style="background-image: url('${post.image}');"></div>
        <div class="banner-text">
            ...
            <h1>${post.title}</h1>
            <p>${new Date(post.date).toLocaleDateString("pl-PL", {day: "numeric", month: "long", year: "numeric"})}</p>
        </div>
    ...
    <div class="breadcrumb">
        <ol class="breadcrumb_list">
            ...
            <li class="breadcrumb_item breadcrumb_item--before breadcrumb_item-active">
                <a href="/aktualnosci?post=${post.id}">${new Date(post.date).toLocaleDateString("pl-PL", {day: "2-digit", month: "2-digit", year: "numeric"})}</a>
            </li>
        </ol>
    </div>
...
function renderNews() {
  ...
  gallery.innerHTML = visibleNews.map(news => `
      ...
      <div class="news-galery-element-text">
        ...
        <div class="news-galery-element-footer">
          <div class="news-galery-element-date">
            ${new Date(news.date).toLocaleDateString("pl-PL", {day: "numeric", month: "long", year: "numeric"})}
          </div>
  ...
  BarInfo.innerHTML = `<div class="showed-bar-S" style="width: ${countBar}%;"></div><div class="showed-bar-H" style="width: calc(100% - ${countBar}%);"></div>`
  loadMoreBtn.style.display = visibleNews.length >= newsData.length ? "none" : "inline-block";
}
